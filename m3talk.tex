...
\section{Saturation-based first order theorem provers}
...
\frame{
	...
	\item Derive new clauses
	
	with a sound and refutation complete calculus
	\item Stop if 
	\begin{enumerate}
	\item time's up or space's out
	\item a proof for unsatisfiability has been found
	\item the set is saturated, hence satisfiable
	\end{enumerate}
...
\begin{example}[Superposition calculus]
\vspace{-1em}
\begin{gather*}
...
\begin{frame}
\begin{example}[Inst-Gen-Eq with unit superpositon]
%
\vspace{-1em}
%
\newcommand{\inferFXC}{
...
%
\center
$\sigma=\{ y\mapsto x \}$, $\tau=\{ x\mapsto \ma \}$, and $\sigma\tau=\{ x\mapsto \ma \}$.
\end{example}
\end{frame}
