...
\item $T ::= V \mid F \mid F ( T,\ldots,T)$\hfill terms
\item $A ::= P \mid P ( T, \ldots, T ) \mid T \foEQ T$ \hfill atoms, i.e.~predicate terms
...
%	
%	}
\item $\ell ::= A \mid \lnot A$\hfill literals
\item $C ::= \emptyclause \mid \ell \mid C \lor \ell$\hfill clauses
\item $S ::= \emptyset \mid S \cup C$\hfill set of clauses

$\{ C_1, \ldots, C_n \} \equiv \forall\ \mcV(C_1) C_1 \land \ldots \forall\ \mcV(C_1) C_1$
...
\frame{
	...
	\item Stop if 
	\begin{enumerate}
	\item a proof for unsatisfiability has been found\hfill{\colG F is a theorem}
	\item the set is saturated, hence satisfiable\hfill{\colG not a theorem} 
...
\begin{remark}
	...
	InstGenEq (Ganzinger, Korovin) are sound and refutational complete with a \MYEM{fair} strategy.
	\end{remark}
\end{frame}
